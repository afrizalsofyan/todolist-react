...
function Navigation({ pageName, isGoBack, isEdit, onClickButton, isFilter, goBackTo, data, isLoading, onSelectSort }) {
  ...
  return (
    <div className='flex flex-col items-center md:grid grid-cols-2 w-full gap-4 md:gap-0'>
      <div className='flex gap-3 items-center w-full justify-center md:justify-start'>
        ...
        {editField ? <input data-cy="activity-title" className='capitalize text-3xl font-bold w-1/2 md:w-2/3 truncate bg-transparent focus:outline-none border-b-2 pb-2 border-black' title={pageName} value={valTitle} onChange={handleChange} /> : <span data-cy="todo-title" className='capitalize text-3xl font-bold max-w-[50%] md:max-w-[66%] truncate bg-transparent' title={isEdit ? valTitle : pageName}>{isEdit ? valTitle : pageName}</span>}
        {isEdit ? <img src={EditIcn} alt='edit-icn' data-cy="todo-title-edit-button" className='cursor-pointer' onClick={handleSave} /> : null}
      </div>
              ...
              {showDropdown ? (
                <>
                  <div className='absolute flex flex-col bg-white top-20 -left-10 shadow-card rounded-[6px] z-10'>
                    {sortMenu.map((e, i) => {
                      return (
                        <div key={'key-sort' + i} className='h-[52px] w-[235px] flex items-center border-b px-[21px] gap-4 capitalize hover:bg-gray-100 cursor-pointer' onClick={() => handleSort(e, i)}><img src={e.img} alt={`${e.name}-icn-sort`} />{e.name}</div>
                      )
                    })}
                  </div>
                </>
